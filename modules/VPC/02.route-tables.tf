...
resource "aws_route_table" "public-rtb" {
  ...
  tags   = merge({ "Name" : "${var.project}-${var.workspace}-PublicRouteTable" }, var.tags)
}
...
resource "aws_route_table" "web-proxy-server-private-rtb" {
  vpc_id = aws_vpc.main.id
  tags   = merge({ "Name" : "${var.project}-${var.workspace}-WebAndProxyServer-PrivateRouteTable" }, var.tags)
}
...
resource "aws_route_table" "database-private-rtb" {
  vpc_id = aws_vpc.main.id
  tags   = merge({ "Name" : "${var.project}-${var.workspace}-Database-PrivateRouteTable" }, var.tags)
}
